...
public class MediaAppWidgetProvider4 extends AppWidgetProvider {
    ...
    static final ComponentName THIS_APPWIDGET =
        new ComponentName("nyanchew.android.music",
                "nyanchew.android.music.MediaAppWidgetProvider4");
    ...
    void notifyChange(NycwMediaPlaybackService service, String what) {
        if (hasInstances(service)) {
            if (NycwMediaPlaybackService.PLAYBACK_COMPLETE.equals(what) ||
                    ...
                    NycwMediaPlaybackService.REPEAT_CHANGED.equals(what) ||
                    NycwMediaPlaybackService.SHUFFLE_CHANGED.equals(what) ||
	...
	final int shuffle_mode = service.getShuffleMode();
        if (shuffle_mode == NycwMediaPlaybackService.SHUFFLE_NONE) {
            views.setImageViewResource(R.id.shuffle, R.drawable.shuffle_off);
        } else if (shuffle_mode == NycwMediaPlaybackService.SHUFFLE_AUTO) {
	...
	final int repeat_mode = service.getRepeatMode();
        if (repeat_mode == NycwMediaPlaybackService.REPEAT_ALL) {
            views.setImageViewResource(R.id.repeat, R.drawable.repeat_all);
        } else if (repeat_mode == NycwMediaPlaybackService.REPEAT_CURRENT) {
    ...
    private void linkButtons(Context context, RemoteViews views, boolean playerActive) {
        ...
        intent = new Intent(NycwMediaPlaybackService.SHUFFLE_ACTION);
        intent.setComponent(serviceName);
        pendingIntent = PendingIntent.getService(context,
                0 /* no requestCode */, intent, 0 /* no flags */);
        views.setOnClickPendingIntent(R.id.shuffle, pendingIntent);

        intent = new Intent(NycwMediaPlaybackService.REPEAT_ACTION);
        intent.setComponent(serviceName);
        pendingIntent = PendingIntent.getService(context,
                0 /* no requestCode */, intent, 0 /* no flags */);
