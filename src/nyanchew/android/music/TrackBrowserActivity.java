...
import nyanchew.android.music.TouchInterceptor.DropListener;
import nyanchew.android.music.TouchInterceptor.RemoveListener;
        ...
        implements View.OnCreateContextMenuListener, MusicUtils.Defs, ServiceConnection
{
    ...
    public void init(Cursor newCursor, boolean isLimited) {
        ...
        f.addAction(NycwMediaPlaybackService.QUEUE_CHANGED);
        if ("nowplaying".equals(mPlaylist)) {
            try {
                int cur = MusicUtils.sService.getQueuePosition();
                setSelection(cur);
                registerReceiver(mNowPlayingListener, new IntentFilter(f));
                mNowPlayingListener.onReceive(this, new Intent(NycwMediaPlaybackService.META_CHANGED));
            ...
            mTrackListListener.onReceive(this, new Intent(NycwMediaPlaybackService.META_CHANGED));
        }
    }
    ...
    private void setTitle() {
        ...
        if (mAlbumId != null) {
            ...
            if (mPlaylist.equals("nowplaying")) {
                if (MusicUtils.getCurrentShuffleMode() == NycwMediaPlaybackService.SHUFFLE_AUTO) {
    ...
    private BroadcastReceiver mNowPlayingListener = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(NycwMediaPlaybackService.META_CHANGED)) {
                getListView().invalidateViews();
            } else if (intent.getAction().equals(NycwMediaPlaybackService.QUEUE_CHANGED)) {
                if (mDeletedOneRow) {
    ...
    private void removeItem() {
        ...
        if ("nowplaying".equals(mPlaylist)) {
            ...
            try {
                if (curpos != nyanchew.android.music.MusicUtils.sService.getQueuePosition()) {
                    mDeletedOneRow = true;
                }
            } catch (RemoteException ex) {
            }
    ...
    protected void onListItemClick(ListView l, View v, int position, long id)
    {
        ...
        if (mTrackCursor instanceof NowPlayingCursor) {
            if (nyanchew.android.music.MusicUtils.sService != null) {
                try {
                    nyanchew.android.music.MusicUtils.sService.setQueuePosition(position);
                    return;
                } catch (RemoteException ex) {
                }
            }
    ...
    private Cursor getTrackCursor(TrackListAdapter.TrackQueryHandler queryhandler, String filter,
        ...
        if (mGenre != null) {
            ...
            if (mPlaylist.equals("nowplaying")) {
                if (nyanchew.android.music.MusicUtils.sService != null) {
                    ret = new NowPlayingCursor(nyanchew.android.music.MusicUtils.sService, mCursorCols);
                    if (ret.getCount() == 0) {
                        finish();
                    }
                } else {
    ...
    static class TrackListAdapter extends SimpleCursorAdapter implements SectionIndexer {
        ...
        public void bindView(View view, Context context, Cursor cursor) {
            ...
            if (nyanchew.android.music.MusicUtils.sService != null) {
                ...
                try {
                    if (mIsNowPlaying) {
                        id = nyanchew.android.music.MusicUtils.sService.getQueuePosition();
                    } else {
                        id = nyanchew.android.music.MusicUtils.sService.getAudioId();
                    }
                } catch (RemoteException ex) {
                }
            }
